...
class FlightDataStream:
    ...
    def get_telemetry(self):
        ...
        return {
            "pitch": math.sin(t * 0.8) * 18.0,            # Degrees (-18 to +18)
            "roll": math.sin(t * 0.5) * 25.0,             # Degrees (-25 to +25)
            "airspeed": 115.0 + math.sin(t * 0.4) * 35.0, # Knots (80 to 150)
            "altitude": 2850.0 + math.sin(t * 0.3) * 600.0,# Feet
            "heading": (t * 4.0) % 360.0,                 # Degrees (0 to 360)
            "turn_rate": math.sin(t * 0.5) * 2.5,         # Deg/sec
            "vsi": math.cos(t * 0.3) * 1200.0,            # Feet per minute
            "ball": math.sin(t * 0.9) * 0.4               # Slip/skid (-1.0 to 1.0)
        }
...
def draw_screw(surface, x, y, size=11):
    """Renders a metallic panel mounting screw with a cross slot."""
...
def draw_gauge_bezel(surface, cx, cy, radius):
    """Renders dark circular instrument housing with mounting screws."""
    # Outer screw mounts
...
def draw_airspeed(surface, cx, cy, r, speed):
    ...
    # Color Arcs (Speed ranges in knots)
    # Mapping: 40 knots at 30 deg, 240 knots at 330 deg (300 deg span)
    def k_to_rad(knots):
        deg = 30 + (knots - 40) * (300.0 / 200.0)
        return math.radians(deg - 90)

    # White arc (Flaps 40-85), Green arc (Normal 55-165), Yellow (Caution 165-200), Red line (200)
...
# -------------------------------------------------------------------
# 2. TOP-CENTER: ATTITUDE INDICATOR
# -------------------------------------------------------------------
def draw_attitude(surface, cx, cy, r, pitch, roll):
    ...
    # Static Roll Index Arc Marks (10, 20, 30, 60 deg)
...
def draw_altimeter(surface, cx, cy, r, altitude):
    ...
    # 10,000 ft Hand (Thin with triangle)
    ...
    # 1,000 ft Hand (Short, broad pointer)
    ...
    # Adjustment Knob (Bottom Left)
    pygame.draw.circle(surface, BEZEL_MID, (cx - r + 15, cy + r - 15), 14)

# -------------------------------------------------------------------
# 4. BOTTOM-LEFT: TURN COORDINATOR
# -------------------------------------------------------------------
def draw_turn_coordinator(surface, cx, cy, r, turn_rate, ball_pos):
    ...
    # Inclinometer (Glass Tube & Slip Ball at bottom)
    ...
    # Rollable Miniature Aircraft Symbol
    ac_surf = pygame.Surface((100, 30), pygame.SRCALPHA)
    # Fuselage & Wings
    pygame.draw.circle(ac_surf, WHITE, (50, 15), 4)
    pygame.draw.line(ac_surf, WHITE, (10, 15), (90, 15), 4) # Wing bar
    pygame.draw.line(ac_surf, WHITE, (50, 5), (50, 25), 3)  # Vertical Tail

    # Rotate mini aircraft based on turn rate
...
# -------------------------------------------------------------------
# 5. BOTTOM-CENTER: HEADING INDICATOR (DIRECTIONAL GYRO)
# -------------------------------------------------------------------
def draw_heading_indicator(surface, cx, cy, r, heading):
    ...
    # Top Heading Index Marker (Orange Lubber Line)
...
# -------------------------------------------------------------------
# 6. BOTTOM-RIGHT: VERTICAL SPEED INDICATOR (VSI)
# -------------------------------------------------------------------
def draw_vsi(surface, cx, cy, r, vsi_rate):
    ...
    # VSI Scale Mapping (0 at 9 o'clock / 180 deg)
    # UP: 0 to 2000 ft/min mapped from 180 deg to 20 deg
    # DOWN: 0 to -2000 ft/min mapped from 180 deg to 340 deg
    ticks = [0, 5, 10, 15, 20]
...
def main():
    ...
    try:
        while True:
            ...
            # Fetch flight telemetry
            data = data_stream.get_telemetry()

            # 1. Background Cockpit Panel
            screen.fill(PANEL_GREY)

            # Draw panel edge screws
            ...
            # 2. Render Outer Housings & Bezels
            for cy in rows:
                for cx in cols:
                    draw_gauge_bezel(screen, cx, cy, r)

            # 3. Render Flight Instruments
            # Row 1
